---
title: "A Survey on LLM Watermarking: Theory and Deployment"
title_zh: LLM水印技术综述：理论与部署
authors: "Huy Phan, Kieu Dang, Ojaswi Dulal, Aiham AL Shukairi, Abby Shine, Chase Garner, Phung Lai"
date: 2026-07-11
pdf: "https://arxiv.org/pdf/2607.10103v1"
tags: ["query:watermarking"]
score: 6.0
evidence: LLM水印综述，涵盖理论与部署
tldr: 大型语言模型（LLM）的广泛使用带来了来源模糊和滥用风险。LLM水印通过在输出中嵌入隐形签名实现溯源与审计。本文从部署视角系统梳理了水印的嵌入位置、检测权限、假设条件和威胁模型，分析了采样偏置、编码、表示和训练等技术的安全-效用权衡，并总结了攻击策略、评估协议和开放挑战。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2607.10103v1/fig-001.webp\", \"caption\": \"\", \"page\": 0, \"index\": 1, \"width\": 1564, \"height\": 963, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2607.10103v1/fig-002.webp\", \"caption\": \"\", \"page\": 0, \"index\": 2, \"width\": 1646, \"height\": 545, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2607.10103v1/fig-003.webp\", \"caption\": \"\", \"page\": 0, \"index\": 3, \"width\": 803, \"height\": 220, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2607.10103v1/fig-004.webp\", \"caption\": \"\", \"page\": 0, \"index\": 4, \"width\": 712, \"height\": 429, \"label\": \"Figure\"}, {\"url\": \"assets/figures/arxiv/2607.10103v1/fig-005.webp\", \"caption\": \"\", \"page\": 0, \"index\": 5, \"width\": 674, \"height\": 198, \"label\": \"Figure\"}]"
motivation: LLM生成文本的溯源难题亟待解决，现有水印文献分类混乱，缺乏部署导向的系统梳理。
method: 按嵌入位置、检测权限、假设条件和威胁模型组织技术体系，综合采样偏置、编码、表示和训练等主流方案。
result: 明确了不同水印方法在可检测性、鲁棒性和分布偏移上的权衡，系统梳理了攻击策略与评估协议。
conclusion: 提供实践指南，指出跨模型迁移、多模态流水线、合谋和治理约束等关键研究方向。
---

## 摘要
大型语言模型（LLMs）越来越多地嵌入高影响力工作流，但其大规模生成流畅文本的能力加剧了来源歧义、模型误用和大规模内容洗白的风险。LLM水印技术通过向模型输出中嵌入不可见签名，已成为一种有前景的技术手段，用于归因、审计和下游信任决策。然而，相关文献发展迅速且不均匀：现有分类常混淆正交设计选择，使得难以比较方法、论证保证或转化研究成果为可部署系统。本综述对LLM水印进行了系统且面向部署的回顾。我们根据实践者必须回答的核心问题组织空间：水印嵌入位置（生成时与训练时、令牌与表示）、谁可检测（公开与私有检测权威）、假设条件（访问logits、采样控制、密钥、模型所有权）以及针对的威胁模型（改写、翻译、摘要、风格迁移、令牌操纵和自适应移除）。我们综合了主要技术家族——包括采样偏置、基于编码的方案、基于表示和训练的方法，并通过可检测性、鲁棒性和分布偏移的视角分析其安全-效用权衡。我们进一步回顾了攻击与规避策略、评估协议与指标（假阳性控制、校准、鲁棒性曲线），以及开放挑战，如跨模型迁移、多模态流水线、合谋和治理约束。最后，我们提供了在真实操作需求下选择水印设计的实用指南，并指出了可靠、负责任的LLM部署所需的研究方向。

## 速览
**TLDR**：LLM生成文本带来溯源模糊和滥用风险，水印技术通过嵌入隐形签名解决归属和审计问题。现有研究分类混乱，本文以部署为导向，从嵌入位置、检测权限、假设条件和威胁模型四个维度组织综述，涵盖采样偏置、代码、表示与训练等方法。系统分析了各类方法在可检测性、鲁棒性和分布偏移上的权衡，并总结了攻击策略和评估协议。最终提供面向真实部署的水印设计选择指南，指出跨模型迁移、多模态管道等开放挑战。 \
**Motivation**：LLM文本大规模生成导致溯源模糊和滥用风险加剧，现有水印研究分类混乱，难以比较方法和指导部署。 \
**Method**：以实践者需求为导向，从嵌入位置、检测权限、假设和威胁模型四维组织，综述采样偏置、代码、表示与训练等主要技术。 \
**Result**：系统分析了各类水印方法在可检测性、鲁棒性和分布偏移上的安全-效用权衡，并梳理了攻击策略与评估协议。 \
**Conclusion**：提供面向真实部署的水印设计选择指南，指出跨模型迁移、多模态管道、共谋攻击等开放挑战。

---

## Abstract
Large language models (LLMs) are increasingly embedded in high-impact workflows, yet their ability to generate fluent text at scale has amplified risks of provenance ambiguity, model misuse, and large-scale content laundering. LLM watermarking, embedding invisible signatures into model outputs, has emerged as a promising technical layer for attribution, auditing, and downstream trust decisions. However, the literature has grown rapidly and unevenly: existing categorizations often mix orthogonal design choices, making it difficult to compare methods, reason about guarantees, or translate research results into deployable systems. This survey provides a systematic, deployment-oriented review of LLM watermarking. We organize the space by the core questions practitioners must answer: where a watermark is embedded (generation-time vs. training-time, token vs. representation), who can detect it (public vs. private detection authority), what is assumed (access to logits, sampling control, secret keys, model ownership), and which threat models are targeted (paraphrasing, translation, summarization, style transfer, token manipulation, and adaptive removal). We synthesize the main families of techniques-including sampling biasing, code-based schemes, representation- and training-based approaches-and analyze their security-utility trade-offs through the lens of detectability, robustness, and distribution shift. We further review attack and evasion strategies, evaluation protocols and metrics (false positive control, calibration, robustness curves), and open challenges such as cross-model transfer, multi-modal pipelines, collusion, and governance constraints. Finally, we provide practical guidance for selecting watermark designs under real operational requirements and identify research directions needed for reliable, accountable LLM deployment.